...
public class TestEncodeMp3 {
    ...
    static float[][] readAudio() throws Exception {
        ...
        System.out.format( "Codec: %d channels, %d layout, %d sampleRate, %d sampleFormat\n" ,
                           cc.channels(),
                           cc.channelLayout(),
                           cc.sampleRate(),
                           cc.sampleFormat() );
        ...
        while( true ) {
            ...
            for( int i = 0; i < chans; i++ ) {
                buf.clear().limit( samps * 2 );
                JavMem.copy( frame.dataPointer( i ), buf );
                buf.flip();
                float[] dst = audio[i];
                
                for( int j = 0; j < len; j++ ) {
                    dst[samplePos+j] = -((float)buf.getShort() / Short.MIN_VALUE);
                }
            }
